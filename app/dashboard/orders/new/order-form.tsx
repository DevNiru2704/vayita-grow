...
export function OrderForm({
  ...
  const [customerId, setCustomerId] = useState<number>(
    initialCustomerId && customers.some((c) => c.customerId === initialCustomerId)
      ? initialCustomerId
      : (customers[0]?.customerId ?? 0),
  );
  ...
  function handleSubmit(event: React.FormEvent) {
    ...
    startTransition(async () => {
      const result = await createOrder({ customerId, items: lines });
      if (result.ok) {
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="max-w-3xl space-y-6">
      <div className="rounded-xl border bg-card p-5">
        <FormField id="order-customer" label="Client" errors={fieldErrors.customerId}>
          <select
            {...fieldAria("order-customer", fieldErrors.customerId)}
            value={customerId}
      ...
      <fieldset className="rounded-xl border bg-card p-5">
            ...
            return (
              ...
              >
                <div className="space-y-1.5">
                  <Label htmlFor={`line-product-${index}`}>Product</Label>
                  <select
        ...
        <p className="mt-5 flex items-center justify-between border-t pt-4 text-sm">
          <span className="text-muted-foreground">
            Unit prices are locked at creation, per order semantics.
          </span>
